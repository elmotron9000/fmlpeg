...
export async function addAudioClipsToVideo(
  ...
  enhancedClips.forEach(clip => {
    if (clip.duration === null) {
      throw new Error(
        `Audio file ${clip.filename} length could not be determined`,
      );
    }
  });
  ...
  if (audioInfo.length > 0) {
    ...
    filters.push(
      ...audioInfo.map((info, index) =>
        audioInfoToComplexFilter(info, index + 1),
      ),
    );
...
function buildAudioInfo(
  ...
  if (currentStart > duration) {
    ...
    console.log({
      currentStart,
      duration,
      lastTimestamp: lastClip.timestamp,
      lastDuration: lastClip.duration,
    });
...
function audioInfoToComplexFilter(
  audioInfo: AudioInfo,
  overall: number,
): string {
  const device = `${audioInfo.index}:a`;
  return `[${device}]atrim=${getRange(
    audioInfo,
  )},asetpts=PTS-STARTPTS[aud${overall}]`;
}
...
function joinAudioFilter(total: number): string {
  const audioFeeds = range(total, +1)
    .map(i => `[aud${i}]`)
    .join("");
